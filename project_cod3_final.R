...
plot(ts_train) #bitcoing time series plot #eda

coin_decomp <- decompose(ts_train)
plot(coin_decomp) #bitcoing time series plot #decomposition of time series#eda
...
#First we find seasonal difference , looks good at lag=3
diff(log(1+coin_sub$mean_price),lag=3) %>% ggtsdisplay()

#THen we come to non seasonal differencing, we see # Transformed time series with ACF and PACF
...
ui <- fluidPage(
              ...
              sidebarLayout( sidebarPanel( selectInput('plotname', 'Select a plot from the dropdown below',
                            ...
                            choices = c('Weighted Price Plot with Time',
                ...
                server <- function(input,output,session){
                  output$timeseries <- renderPlot({
                    if(input$plotname == 'Weighted Price Plot with Time') {
                      plot.ts(coin$Weighted_Price, main = 'Weighted_Price Plot with Time', xy.labels = c("TIme","Weighted_Price"))
                    }
                    
                    if(input$plotname == 'Exploratory Data Analysis') {
                      ggplot(data = price_with_time, aes(x = year, y = percent_NAs)) +
                        geom_bar(stat = "identity", width = 0.3) 
